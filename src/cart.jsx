...
console.log("checking id",eachItemPrice,id)
    ...
    return(
    <div  key={id} className="box-border text-[13.5px] font-semibold  text-slate-700  flex items-center align-middle  justify-between px-2 mt-2 
...
const Cart = ({fromHeader,setIsCartClicked})=>{
  ...
  return(
    ...
    <div key={"cartbody1"} className={`flex w-screen bg-gray-200 p-2 lg:align-top lg:items-start lg:h-screen items-center flex-col-reverse  z-50 text-black ${fromHeader? (pageColour!="white"?"h-full bg-transparent  w-full  text-black":"h-full bg-transparent w-full "):pageColour=="white"?" w-screen lg:flex-row ":"  w-screen lg:flex-row  text-white"  } `}>
...
:<button onClick={()=>handleData(cartItems,totals,Dispatch,setSuccessfulPayment,setOrderId,isUsing20PercentOff)} className="text-lg font-semibold max-w-lg p-2 py-4 bg-orange-500 rounded-md active:bg-orange-800 hover:shadow-lg shadow-black  text-white m-2 mx-4">Pay  ₹ {Math.round(totals)}</button>}
</>:null}
